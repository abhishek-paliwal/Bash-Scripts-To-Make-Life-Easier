...
function delete_CSVs_older_than_2_days () {
    ...
    if [ -z "$files_to_delete" ] ; then echo "NOTE: No files to delete." ; else figlet -t -f cybermedium "NOTE: Files will be deleted." ; read -p "If OKAY, press ENTER to DELETE ..." ; rm $files_to_delete ; fi
}
#####
