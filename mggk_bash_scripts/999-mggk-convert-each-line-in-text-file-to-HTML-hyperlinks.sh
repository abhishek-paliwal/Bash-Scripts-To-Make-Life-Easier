...
function FUNCTION_CONVERT_TEXT_LINES_TO_HYPERLINKS () {
  ...
  echo "<h1>List of all Lines converted to URLs (Localhost + MGGK)</h1>" > $OUTPUT_HTMLFILE ;
  ...
  do
    ...
    line_cleaned_mggk=$(echo $line | sed 's+http://localhost:1313/+https://www.mygingergarlickitchen.com/+g') ;
    echo "<h3>$count. <a target='_blank' href='$line'>$line_cleaned</a> // <a target='_blank' href='$line_cleaned_mggk'>(See this on MGGK)</a> </h3>(Original link = $line)" >> $OUTPUT_HTMLFILE ;
  ((count++)) ;  
  done < "$input_file" 
  ####################################
