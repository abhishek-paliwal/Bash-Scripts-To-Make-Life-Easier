...
<h1>Finding maching Post Titles + URLs with the words in the given text file (= $FILE1)</h1>
...
##
#grep -irh 'url: ' $HUGODIR/ | sed 's/url: //g' | sed 's+/++g' | sed 's/-/ /g' | tr ' ' '\n' | sed 's/"//g' | grep -v '^$' | sort | uniq > $FILE2A ;
grep -irh '^title: ' $HUGODIR/ | awk '{print tolower($0)}' | sed 's/title: //g' | sed 's+/++g' | sed 's/-/ /g' | sed 's/"//g' | sed "s/'//g" | sed 's/\.//g' | sed 's/(//g' | sed 's/)//g' | sed 's/\[//g' | sed 's/\]//g' | sed 's/,//g' | sed 's/\?//g' | sed 's/|//' | sed 's/–//g' | sed 's/—//g' | tr ' ' '\n'  | grep -v '^$' | sort | uniq > $FILE2A ;
...
function FUNCTION_FIND_COMMON_WORDS_IN_URLS () {
    ...
    ####
        ...
        if [ $lines_matched -le 200 ] && [ $lines_matched -ge 1 ]  ; then
            ...
            echo "<mark><strong>MATCHED POST TITLES (lowercase) ====> WITH THEIR URLS:</strong></mark>" >> $FILE_FINAL ;
